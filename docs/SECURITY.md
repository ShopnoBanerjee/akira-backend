...
| 28 | The deploy pipeline (P25) | A push to `main` reaching production unreviewed; deploy credentials in the repo; a migration re-run or an edited migration corrupting the schema | Deploy jobs run only for `main`, only with `DEPLOY_ENABLED=true`, and only after a required reviewer approves the `production` environment; tokens are environment secrets (Render API key and service id, Cloudflare token, pooler URL) and never appear in logs; `scripts/migrate.py` applies each file once, in a transaction, refuses an edited applied file and a hand-migrated database without a baseline; `schema_migrations` is RLS-forced with no grants; both jobs curl what they deployed and fail otherwise | `.github/workflows/ci.yml` in both repos; `scripts/migrate.py` | `tests/test_migrate.py` — from zero, twice, only-new, rollback, edited file, baseline; the first real run is the owner's approval |
...
| 29 | Organisations (P26a, D33) | An owner, manager or leaked key of one tenant reading or changing another's outlets, people, catalogue, menu, recipes, settings or events; the platform account acting inside a tenant; a password-only session on the accounts that can do the most damage; a stranger signing up and gaining a profile | Reach is computed from the organisation in `CurrentUser` (owners: the tenant's outlets; managers: memberships inside it; `visible_outlet_ids` is None only for the platform admin) and every service reads that; organisation-level tables filter on `organisation_id is null or = caller's`; RLS helpers redefined so every outlet policy is tenant-safe, plus read policies per content table; platform admin writes outside `/platform` refused in `deps.py` and its reads audited as `action='read'`; `aal2` required for the platform admin and for owners of onboarded organisations (`mfa-required` problem type, only `GET /users/me` and probes exempt); unknown subjects refused without writing; `platform_admin` grantable by no API call; outlet and people caps per organisation | `app/core/deps.py` (`CurrentUser`, `_mfa_gate`, `_platform_admin_gate`), `0026_organisations.sql`, every domain service, `scripts/create_platform_admin.py` | `tests/test_tenancy.py` — the fence, the loader, the MFA gate, the platform gate, two organisations through the services, and RLS as the other tenant; `tests/test_rls.py` and `test_migrations.py` from zero |
